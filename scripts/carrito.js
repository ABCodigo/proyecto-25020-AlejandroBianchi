...
function crearFilaProducto(producto) {
    ...
    return `
        <tr>
            ...
            <td>
                <img src="${producto.image}" alt="${producto.nombre}" style="height: 80px; width: auto; object-fit: contain;">
            </td>
...
function eventosFila() {
    ...
    document.querySelectorAll('.cantidad-producto').forEach(input => {
        input.addEventListener('change', () => {
            ...
            const productId = input.id;  //se le saco el parseInt a parseInt(input.id) pues con el parseInt no funcionaba;
            const nuevaCantidad = input.value; //se le saco el parseInt a parseInt(input.value);
...
function actualizarTotales() {
    ...
    filas.forEach(fila => {
        ...
        if (input) {
            const productId = (input.id); // se sacoel parseInt(input.id); //const productId = parseInt(input.dataset.id);
            const producto = carrito.find(item => item.id === productId);
            if (producto) {
