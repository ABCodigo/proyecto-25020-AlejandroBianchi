...
function cargarProductosCarrito() {
    ...
    if (carrito.length === 0) {
        ...
        document.querySelector('#tabla_carrito').innerHTML = '<tr><td colspan="6" style="text-align: center; padding: 20px;">Tu carrito está vacío. Agrega productos desde la seccion<a href="./productos.html">  Productos</a>.</td></tr>';
    } else {
...
function crearFilaProducto(producto) {
    ...
    return `
        <tr>
            <td>
                <button id="${producto.id}" class="remove-btn"><i class="fa-solid fa-trash-can"></i></button>
            </td>
            <td>
                <img src="${producto.image}" alt="${producto.name}" style="height: 80px; width: auto; object-fit: contain;">
            </td>
            <td>${producto.name}</td>
...
}    // Esto lo que hace es argregar a todos (All) los queryselector donde esta el ID Total (#total) el valor de subtotal que viene/ que se obtuvo de subtotalcalculado. El argumento subtotalcalculado se pasa como parametro de esta funcion actualizarTotalCarrito.
...
function eventosFila() {
    ...
    // Eventos para botones de eliminar  ---> se refiere a eliminar todo el producto seleccionado
    ...
    document.querySelectorAll('.cantidad-producto').forEach(input => {
        input.addEventListener('change', () => {
            ...
            const productId = parseInt(input.id);  //se le saco el parseInt a parseInt(input.id) pues con el parseInt no funcionaba;
            const nuevaCantidad = parseInt(input.value); //se le saco el parseInt a parseInt(input.value);
...
function actualizarTotales() {
    ...
    filas.forEach(fila => {
        ...
        if (input) {
            const productId = parseInt(input.id); // se sacoel parseInt(input.id); //const productId = parseInt(input.dataset.id);
            const producto = carrito.find(item => item.id === productId);
            if (producto) {
